...
'Begin, Menachem', 'Belloc, Hilaire', 'Bellow, Saul',
 ...
 'Biden, Joseph', 'Bierce, Ambrose', 'Biko, Steve', 
...
const inventors1500s = inventors.filter((birthday) => {
  return birthday.first[0] === 'a' && birthday.year <= 1599
});

const inventorsNames = inventors.map((element) => {
  return `${element.first} ${element.last}`
})

const inventorsDates = inventors.sort((a, b) => {
  if(a.year > b.year) {
    return 1;
  } else {
    return -1;
...
inventorsLast = inventors.sort((a,b) => {
